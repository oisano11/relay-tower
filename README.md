...
2. **多渠道分流影响 Prompt Cache**：同一上下文在不同服务商或节点间分流，可能降低提示词缓存命中率，增加 Token 费用和延迟；
3. **突发断流与宕机无法离机切线**：深夜上游突然宕机或被封禁，运维无法立刻开电脑登录后台切换通道；
4. **客户端热切换成本高**：传统中转系统切换渠道需要客户端重启或重新配置。

**中转塔台 (Relay Tower)** 是开源大模型中转网关与监控调度系统，包含**上游倍率监控**、**统一反向代理网关 (`/v1`)**、**Telegram 手机端切线机器人**以及 **Sub2API 容器化部署配置**。
...
| :--- | :---: | :---: | :---: |
| **上游进货倍率探针与差分感知** | ❌ 无感知 | ❌ 仅配置固定售价 | ✅ **定时探测与差分告警，默认每 5 分钟检查倍率** |
| **高危涨价强告警与备选熔断** | ❌ 无告警 | ❌ 无告警 | ✅ **主力涨价声光+Telegram强提醒，一键切至低价备选** |
| **Prompt Cache 单主路由亲和** | 取决于路由配置 | 取决于路由配置 | ✅ **适用分组维持冷备，减少跨通道分流** |
| **Telegram 手机端双向交互切线** | ❌ 无 | ❌ 仅单向被动推送 | ✅ **手机端弹出 Inline Keyboard，点击按钮秒级切线** |
| **统一入口切换 (`/v1`)** | 取决于配置 | 取决于配置 | ✅ **切换后续请求的上游，无需修改客户端入口** |
...
### 2. 🚨 差分调价感知与高危涨价强告警
- **定时差分检测**：每轮倍率检查比较变化幅度（如 `0.65x -> 1.10x (+69.2%)`），默认周期为 5 分钟；通道探活默认每 3 小时执行，可手动触发检查。检测速度受周期及上游响应影响。
- **全维度立体报警**：屏幕居中强警示弹窗 + Web Audio 合成声效警报 + 完整调价审计日志。
- **主力通道涨价一键熔断**：当正在出海的主力通道被上游暗度陈仓涨价时，系统自动触发红色高危警报，并在消息界面提供“一键切换到更低倍率备选通道”快捷按钮。

### 3. 🔒 单主调独占与 Prompt Cache 路由亲和
- **减少双开分流**：适用分组保留一个主调，同组独立通道设为冷备 (`schedulable=false, priority=10`)；跨组共享渠道自动保留全局调度开关，仅调低组内优先级 (`priority=10`)，杜绝跨组误杀，完全支持一个渠道挂载多个业务分组。
- **缓存亲和**：尽量将后续请求交给同一上游通道，减少跨通道分流。上游仍可在内部调度不同实例；实际缓存命中率和节省金额取决于上游缓存策略、请求内容与有效期，不保证物理实例固定或特定命中率。
- **原子互斥交接**：切线操作（手动切换或自动熔断）执行原子 SQL 事务，瞬间关停旧主调、激活新主调，切线瞬间零分流冗余。

### 4. ⚡ 统一反向代理网关 (Smart Proxy Gateway)
- **统一接入终结点**：对外开放统一端口 `http://localhost:3300/v1`。
- **统一入口切换**：客户端（Claude Code CLI、Cursor、CC Switch、Chatbox、NextChat）配置此端口后，无需修改入口即可让后续请求使用新主调。已有请求不会迁移，故障请求仍可能需要重试。
...
将常用 AI 工具的 Base URL 指向中控台网关 `http://localhost:3300/v1`，即可使用缓存路由亲和与统一入口切换。

> [!TIP]
> **网关密钥获取**：示例中的 `sk-your-relay-key` 为网关接入密钥（Gateway API Key），可在控制台右上角【⚙️ 系统管理 → 🛡️ 安全设置与改密】中一键查看、复制或重置。若客户端与服务部署在同一台机器通过 `127.0.0.1` 访问，系统默认免密放行。
...
### 2. 管理员一键免密绑定
- 在 Telegram 中私聊机器人，使用 `/bind <控制台管理密码>` 绑定，或在控制台配置管理员个人 Telegram ID；
- `/start` 不会自动授予管理员权限。通知群仅接收消息，加入通知群不会使群成员获得操作权限。
...
> **答**：单主调减少跨上游通道分流，有助于保持缓存亲和性。系统完全支持一个渠道属于多个业务组，对跨组共享账号智能保留全局调度开关并仅降级组内优先级，防止误杀其他分组。

### 当前架构边界

数据库和 Redis 同步仍包含同步子进程调用，慢查询或 SSH 等待可能阻塞网关事件循环。异步数据库访问、后台任务与网关分进程属于后续架构工作。本次修复计划与验收范围见 [修复计划](docs/repair-plan.md)；镜像启动检查已加入 CI，实际部署需以运行结果为准。
...
### Q3: 现有生产中转站如何平滑接入本项目？
> **答**：您可以仅部署 `upstream-monitor`。进入该目录执行 `node server.js`，通过中控台导入渠道配置，并将开发工具的 Base URL 切换为 `http://your-server:3300/v1`，即可使用倍率监控与统一入口切线。迁移前应确认客户端兼容性及所需数据库连接。
